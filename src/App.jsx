...
const T = {
  ...
  en: {
    ...
    nav: { overview: "Overview", orders: "Orders", cars: "Cars", zones: "Zones",
      ...
      hiring: "Rep Hiring", shifts: "Rep Performance", tickets: "Tickets", circulars: "Circulars", pulse: "Rep Pulse", iqama: "Residency", payrollRuns: "Payroll Runs", chat: "Messaging", messages: "Messages", leaderboard: "Leaderboard", audit: "Audit Log", hall: "Hall of Fame", charter: "Internal Charter", checklist: "Launch Checklist", growth: "Company Growth", security: "Security", permissions: "Permissions Map", mystory: "My Story" },
...
const ROLES = [
  { id: "gm", icon: Crown, accent: "#E4B85C", glow: "228,184,92", staff: 6, shape: "octagon",
    ...
    nav: ["overview", "growth", "checklist", "leaderboard", "hall", "pulse", "payrollRuns", "audit", "permissions", "tickets", "chat", "circulars", "attendance", "reports", "settings"] },
...
function ExpensesPage({ lang, role, expenses, setExpenses }) {
  const t = T[lang], x = EX[lang];
  const catOpts = Object.keys(EXP_LABEL).map((k) => ({ value: k, label: EXP_LABEL[k][lang] }));
  const add = (v) => setExpenses((prev) => [...prev, { k: v.k, v: +v.amt || 0, date: riyadhToday() }]);
  const total = expenses.reduce((a, b) => a + b.v, 0);
  const byCat = Object.keys(EXP_LABEL).map((k) => ({ k, v: expenses.filter((e) => e.k === k).reduce((a, e) => a + e.v, 0) }));
  return (
    <div className="space-y-4">
      <QuickAddPanel lang={lang} role={role} title={t.k.expense} icon={Wallet} onSubmit={add}
        fields={[
          { key: "k", label: t.c.exp, type: "select", options: catOpts },
          { key: "amt", label: t.h.amount, type: "number", required: true },
        ]} />
...
function PeoplePage({ lang, role, employees, setEmployees, setAuditLog, actingEmployee }) {
  ...
  const [form, setForm] = useState({ name: "", dept: "gm", position: "", photo: null, isSupervisor: false, password: "" });
  const [err, setErr] = useState("");
  ...
  const submit = () => {
    ...
    setEmployees((prev) => [...prev, { id: "EMP-" + Date.now(), name: form.name, dept: form.dept, position: form.position,
      photo: form.photo, isSupervisor: form.dept === "ops" && form.isSupervisor, password: form.password }]);
    logAudit(setAuditLog, role, "employee_add", `${form.name} (${form.dept})`, actingEmployee);
    setForm({ name: "", dept: "gm", position: "", photo: null, isSupervisor: false, password: "" });
  };
...
const PAGES = {
  ...
  permissions: PermissionsMapPage, mystory: MyStoryPage,
};
const NAV_ICON = {
  ...
  security: ShieldAlert, permissions: ShieldCheck, mystory: Sparkles,
};
...
function Shell({ lang, setLang, role, user, activeRep, onSwitch, onLogout, reduce, setReduce, reps, setReps, reports, setReports,
  ...
  deptPasswords, setDeptPasswords, deptSecurity, setDeptSecurity, query, setQuery }) {
  const t = T[lang], x = EX[lang];
